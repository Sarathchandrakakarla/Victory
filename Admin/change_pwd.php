...
if (isset($_POST['change'])) {
  ...
  if ($old == $new) {
    ...
    $res = mysqli_query($link, "SELECT * FROM `admin` WHERE Admin_Id_No = '$id'");
    if (mysqli_num_rows($res) == 1) {
      $row = mysqli_fetch_assoc($res);
      $opasshash = $row['Admin_Hash'];
      if (password_verify($old, $opasshash)) {
        $npasshash = password_hash($new, PASSWORD_DEFAULT);
        if (mysqli_query($link, "UPDATE admin SET Admin_Hash = '$npasshash', Admin_Password = '$new' WHERE Admin_Id_No = '$id'")) {
