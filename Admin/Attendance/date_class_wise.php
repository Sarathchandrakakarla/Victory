                ...
                foreach ($classes as $class) {
                    foreach ($sections as $section) {
                        if (mysqli_num_rows(mysqli_query($link, "SELECT * FROM `student_master_data` WHERE Stu_Class = '$class' AND Stu_Section = '$section'")) != 0) {
                            array_push($working_classes, $class . ' ' . $section);
                        }
                    }
                }
                $working_classes = array_chunk($working_classes, count($working_classes) / 3);
                foreach ($working_classes as $column) {
                    echo '<div class="col-lg-2">';
                    foreach ($column as $working_class) {
                        echo '<input type="checkbox" name="columns[]" id="' . $working_class . '" value="' . $working_class . '"> <label for="' . $working_class . '">' . $working_class . '</label><br>';
                    }
                    echo '</div>';
                }
                ?>
              ...
              document.getElementById('" . strtolower($stu_type) . "').checked = true;
                        ...
                        if (isset($_POST['columns'])) {
                            ...
                            foreach ($cols as $col) {
                                ...
                                $class = substr($col, 0, strlen($col) - 1);
                                $section = $col[strlen($col) - 1];
                            ...
                            echo '<script>alert("Select Atleast One Class!");</script>';
                        }
                        /* 
                        if ($_POST['Class']) {
                            $class = $_POST['Class'];
                            ...
                            $flag = true;
                            $sql = "SELECT Id_No FROM `student_master_data` WHERE Stu_Class = '$class'";
                            if ($class_type == "Section_Wise") {
                                if ($_POST['Section']) {
                                    $section = $_POST['Section'];
                                    echo "<script>document.getElementById('sec').value = '" . $section . "'</script>";
                                    $sql = "SELECT Id_No FROM `student_master_data` WHERE Stu_Class = '$class' AND Stu_Section = '$section'";
                                    $flag = true;
                                } else {
                                    $flag = false;
                        ...
                        if ($flag) {
                            $students_list = mysqli_query($link, $sql);
                            if (mysqli_num_rows($students_list) == 0) {
                                echo "<script>alert('Invalid Class and Section!')</script>";
                            } else {
                                $all_ids = [];
                                while ($students_row = mysqli_fetch_assoc($students_list)) {
                                    array_push($all_ids, $students_row["Id_No"]);
                                }
                                $date = format_date($date);
                                //Arrays
