...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (isset($_POST['add'])) {
    ...
    if (isset($_POST['Author'])) {
      ...
      $author = 'School'; // hardcoded or from session
    }
    ...
    $insert = mysqli_query($link, "INSERT INTO posts (Title, Description, Body, Cover_Photo, Media, Links, Author, Posted_On) VALUES 
          ('$title', '$desc', '$body', '', '[]', '', '$author', '$postedOn')");
    if (!$insert) {
    ...
    $update = mysqli_query($link, "UPDATE posts SET Cover_Photo='$coverFileName', Media='" . mysqli_real_escape_string($link, $mediaJson) . "', Links='" . mysqli_real_escape_string($link, $linksString) . "' WHERE Post_Id=$postId");
