...
if (isset($_POST['Send'])) {
    ...
    $sql = mysqli_query($link, "SELECT Mobile FROM `employee_master_data` WHERE Emp_Id = '" . $_SESSION['UserName'] . "'");
    $sql1 = mysqli_query($link, "SELECT * FROM `faculty` WHERE Id_No = '" . $_SESSION['UserName'] . "'");
...
if (isset($_POST['Verify'])) {
    if (!$_SESSION['OTP']) {
        ...
        if ($user_otp != $generated_otp) {
            ...
            $update_query = mysqli_query($link, "UPDATE `faculty` SET Password = '$newPassword', Fac_Hash = '$hash' WHERE Id_No = '" . $_SESSION['UserName'] . "'");
            if ($update_query) {
    ...
    <div class="container col-lg-4">
        <div class="wrapper">
            ...
            <form action="" method="post">
                ...
                <?php if (!$_SESSION['Id_No']) { ?>
                    <div class="pass"><a href="faculty_login.php">Login</a></div>
                <?php } else { ?>
